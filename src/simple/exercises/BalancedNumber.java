...
public class BalancedNumber {
    //    A balanced number is a number where the sum of digits to the left of the middle digit(s) and the sum of digits to
//    the right of the middle digit(s) are equal.
//
//    If the number has an odd number of digits, then there is only one middle digit. (For example, 92645 has one middle
//    digit, 6.) Otherwise, there are two middle digits. (For example, the middle digits of 1301 are 3 and 0.)
//
    ...
    public static String balancedNum(long number) {
        ...
        if (digits.size() % 2 == 0) {
            for (int i = 0; i < digits.size() / 2 - 1; i++) {
                leftSum += digits.get(i);
            }
            for (int i = digits.size() / 2 + 1; i < digits.size(); i++) {
                rightSum += digits.get(i);
            }
        }
